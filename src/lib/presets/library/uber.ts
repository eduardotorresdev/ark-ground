	...
	preset: {
		...
		graph: snapshot(
			[
				...
				node('service-1', 'service', { label: 'Viagem', language: 'go', capacity: 1500 }),
				node('service-2', 'service', { label: 'Motorista', language: 'go', capacity: 1500 }),
				node('service-3', 'service', { label: 'Pagamentos', language: 'java', capacity: 1500 }),
				node('broker-1', 'broker', { label: 'Eventos de viagem', mode: 'work-queue' }),
				node('database-1', 'database', { label: 'Viagem DB', engine: 'postgres' }),
				node('database-2', 'database', { label: 'Motorista DB', engine: 'postgres' }),
				node('database-3', 'database', { label: 'Pagamentos DB', engine: 'postgres' })
			],
			[
